...
#######################################################
#  LINUX ON TERMUX - Multi-Distro Installer v3.4
#
#  Distributions:
...
# v3.2 FIX: Use builtin read -t instead of sleep in spinner
# When apt upgrade replaces libcrypto.so.3, /usr/bin/sleep dies
# read -t is a bash builtin and never affected by library upgrades
spinner() {
    local pid=$1
    local msg=$2
    local spin='⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏'
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        # v3.2: read -t instead of sleep — survives library upgrades
...
# v3.2 FIX: proot_run now captures exit code, logs failures, always returns 0
proot_run() {
    local output
    local rc=0
    output=$(proot-distro login "$PROOT_DISTRO" -- bash -c "$1" 2>&1) || rc=$?
    if [ $rc -ne 0 ]; then
        log "PROOT_RUN FAILED (rc=$rc): $1"
        log "OUTPUT: $output"
    fi
    return 0
}

# v3.2 FIX: proot_run_verbose now returns the actual exit code so callers can check
proot_run_verbose() {
...
detect_device() {
    ...
    echo ""
    # v3.2: use read instead of sleep (sleep may be broken after apt upgrade)
...
step_termux_base() {
    ...
    # v3.2 FIX: Run pkg update/upgrade WITHOUT spinner
    # During upgrade, libcrypto.so.3 can be replaced which kills sleep
    # and causes the spinner to spam errors. Run directly instead.
    echo -e "  ${YELLOW}⏳${NC} Updating package lists..."
    yes | pkg update -y 2>&1 | tail -3
    echo -e "  ${GREEN}✓${NC} Package lists updated"

    echo -e "  ${YELLOW}⏳${NC} Upgrading installed packages (this may break sleep temporarily)..."
    yes | pkg upgrade -y 2>&1 | tail -5
...
step_install_distro() {
    ...
    echo "$arch" | grep -qi "arm64\|aarch64" && is_arm64=true

    # ---- ARM64 FALLBACKS ----
    # proot-distro registry only has amd64 images for Arch and Kali.
    # On arm64 devices, we must download the arm64 rootfs directly.

    if [ "$DISTRO_ID" == "arch" ] && [ "$is_arm64" == true ]; then
        echo -e "  ${CYAN}▸${NC} arm64 device — using Arch Linux ARM rootfs"
        local tarball="${PREFIX}/tmp/archlinuxarm-aarch64.tar.gz"

        echo -e "  ${YELLOW}⏳${NC} Downloading Arch Linux ARM aarch64 rootfs..."
        wget -q --show-progress \
            "http://os.archlinuxarm.org/os/ArchLinuxARM-aarch64-latest.tar.gz" \
            -O "$tarball" 2>&1 | tail -3

        if [ ! -f "$tarball" ] || [ $(stat -c%s "$tarball" 2>/dev/null || echo 0) -lt 1000000 ]; then
            echo -e "  ${RED}✗${NC} Failed to download Arch Linux ARM rootfs"
            echo -e "  ${GRAY}  Try manually: wget http://os.archlinuxarm.org/os/ArchLinuxARM-aarch64-latest.tar.gz${NC}"
            return 1
        fi

        proot-distro install "$tarball" --override-alias archlinux 2>&1 | tail -5
        local rc=${PIPESTATUS[0]}
        rm -f "$tarball"

        if [ $rc -ne 0 ]; then
            echo -e "  ${RED}✗${NC} Failed to install Arch Linux ARM"
            return 1
        fi
        echo -e "  ${GREEN}✓${NC} Arch Linux ARM (aarch64) rootfs installed"

    elif [ "$DISTRO_ID" == "kali" ] && [ "$is_arm64" == true ]; then
        echo -e "  ${CYAN}▸${NC} arm64 device — using Kali Linux ARM rootfs"
        local tarball="${PREFIX}/tmp/kalifs-arm64.tar.xz"

        echo -e "  ${YELLOW}⏳${NC} Downloading Kali Linux ARM64 rootfs..."
        wget -q --show-progress \
            "https://kali.download/nethunter-images/current/rootfs/kalifs-arm64-minimal.tar.xz" \
            -O "$tarball" 2>&1 | tail -3

        if [ ! -f "$tarball" ] || [ $(stat -c%s "$tarball" 2>/dev/null || echo 0) -lt 1000000 ]; then
            echo -e "  ${RED}✗${NC} Failed to download Kali ARM64 rootfs"
            echo -e "  ${GRAY}  Try manually: wget https://kali.download/nethunter-images/current/rootfs/kalifs-arm64-minimal.tar.xz${NC}"
            return 1
        fi

        proot-distro install "$tarball" --override-alias kali 2>&1 | tail -5
        local rc=${PIPESTATUS[0]}
        rm -f "$tarball"

        if [ $rc -ne 0 ]; then
            echo -e "  ${RED}✗${NC} Failed to install Kali ARM64"
            return 1
        fi
        echo -e "  ${GREEN}✓${NC} Kali Linux ARM64 rootfs installed"

    else
        # All other distros: use proot-distro registry (ubuntu, debian, fedora, alpine have arm64)
        proot-distro install "$PROOT_DISTRO" 2>&1 | tail -5
        local rc=${PIPESTATUS[0]}

        if [ $rc -ne 0 ]; then
            echo -e "  ${RED}✗${NC} Failed to install ${DISTRO_NAME} rootfs"
    ...
    # Verify the container actually exists before continuing
    # v3.4: Use 'proot-distro login' test instead of grep on 'proot-distro list'
    # (the list format is inconsistent and grep matches uninstalled entries too)
    if ! proot-distro login "$PROOT_DISTRO" -- echo ok >/dev/null 2>&1; then
        echo -e "  ${RED}✗${NC} Container '$PROOT_DISTRO' is not working — install may have failed"
        echo -e "  ${GRAY}  Run 'proot-distro list' to check installed distributions${NC}"
        return 1
    fi
}
...
# v3.2 FIX: Install GNOME with error detection and automatic repair
# If dpkg fails (common in proot), run dpkg --configure -a + apt --fix-broken
_install_de_ubuntu() {
    ...
    # v3.2: If install failed, repair automatically
    if [ $rc -ne 0 ]; then
        echo -e "  ${YELLOW}⚠${NC} Some packages failed — running repair..."
        proot_run_verbose "
            export DEBIAN_FRONTEND=noninteractive
            dpkg --configure -a 2>&1 | tail -3
            apt-get install --fix-broken -y 2>&1 | tail -3
            apt-get install -y --no-install-recommends \
                ubuntu-desktop-minimal \
                gnome-terminal nautilus gedit eog evince \
                gnome-tweaks gnome-shell-extensions \
                dbus-x11 at-spi2-core xdg-utils fonts-noto \
                glib-networking gsettings-desktop-schemas 2>&1 | tail -5
        "
        rc=$?
    fi

    if [ $rc -ne 0 ]; then
        echo -e "  ${RED}✗${NC} GNOME install had errors — desktop may be incomplete"
        echo -e "  ${GRAY}  Run 'bash ~/shell-linux.sh' then 'apt --fix-broken install' manually${NC}"
    else
...
_install_de_arch() {
    ...
    # v3.3: Arch Linux ARM requires keyring init before pacman -S works
    echo -e "  ${YELLOW}⏳${NC} Initializing pacman keyring (first time setup)..."
    proot_run_verbose "
        pacman-key --init 2>&1 | tail -2
        pacman-key --populate archlinuxarm 2>&1 | tail -2
    "
    ...
    echo -e "  ${YELLOW}⏳${NC} Installing Hyprland + desktop packages..."
    proot_run_verbose "
        pacman -S --noconfirm \
            hyprland \
            waybar \
    ...
    local rc=$?
    if [ $rc -ne 0 ]; then
        echo -e "  ${YELLOW}⚠${NC} Some Arch packages failed — this is common on ARM"
        echo -e "  ${GRAY}  Run 'bash ~/shell-linux.sh' then 'pacman -S hyprland waybar' manually${NC}"
    else
...
_install_de_debian() {
    ...
    # v3.4 FIX: Install plasma-workspace first (provides startplasma-x11)
    # Remove 'spectacle' — not in Debian 12 default repos
    proot_run_verbose "
        export DEBIAN_FRONTEND=noninteractive
        apt-get update -qq
        apt-get install -y --no-install-recommends \
    ...
    if [ $rc -ne 0 ]; then
        ...
        proot_run_verbose "
            ...
            dpkg --configure -a 2>&1 | tail -3
            apt-get install --fix-broken -y 2>&1 | tail -3
            # Retry critical packages
            apt-get install -y --no-install-recommends \
                plasma-workspace kde-plasma-desktop plasma-nm plasma-pa \
                dolphin konsole kate ark okular \
                dbus-x11 fonts-noto 2>&1 | tail -5
        "
    fi

    # v3.4: Verify startplasma-x11 actually exists
    if ! proot-distro login "$PROOT_DISTRO" -- which startplasma-x11 >/dev/null 2>&1; then
        echo -e "  ${YELLOW}⚠${NC} startplasma-x11 not found — trying plasma-workway alone..."
        proot_run_verbose "
            export DEBIAN_FRONTEND=noninteractive
            apt-get install -y --no-install-recommends plasma-workway 2>/dev/null
            apt-get install -y --no-install-recommends plasma-desktop 2>/dev/null
        "
    fi
...
_install_de_kali() {
    ...
    # v3.3: Nethunter minimal rootfs may need Kali repos added first
    proot_run_verbose "
        export DEBIAN_FRONTEND=noninteractive

        # Ensure Kali repos are configured (Nethunter minimal may be missing them)
        ...
        # Install Kali archive keyring first
        apt-get update -qq 2>&1 | tail -3
        apt-get install -y --no-install-recommends kali-archive-keyring 2>/dev/null
...
proot-distro login ${PROOT_DISTRO} --shared-tmp -- bash -c "
  ...
  # v3.2: Ensure GSettings schemas are found (fixes 'No GSettings schemas' crash)
  export GSETTINGS_SCHEMA_DIR=/usr/share/glib-2.0/schemas
  export XDG_DATA_DIRS=/usr/local/share:/usr/share

  # v3.2: Compile schemas if missing
  if [ -d /usr/share/glib-2.0/schemas ] && ! ls /usr/share/glib-2.0/schemas/*.compiled 2>/dev/null; then
...
STARTEOF
    ...
    # v3.2 FIX: Also copy to script dir so ./start-linux.sh works
    cp ~/start-linux.sh "${SCRIPT_DIR}/start-linux.sh" 2>/dev/null
    cp ~/start-linux.sh ~/linux-on-termux/start-linux.sh 2>/dev/null
...
STOPEOF
    ...
    cp ~/stop-linux.sh "${SCRIPT_DIR}/stop-linux.sh" 2>/dev/null
    cp ~/stop-linux.sh ~/linux-on-termux/stop-linux.sh 2>/dev/null
...
SHELLEOF
    ...
    cp ~/shell-linux.sh "${SCRIPT_DIR}/shell-linux.sh" 2>/dev/null
    cp ~/shell-linux.sh ~/linux-on-termux/shell-linux.sh 2>/dev/null
...
main() {
    ...
    # v3.4: Stop if distro install fails — no point continuing without a rootfs
    step_install_distro || {
        echo -e "\n  ${RED}Installation aborted — distro rootfs failed to install.${NC}"
        echo -e "  ${GRAY}Check the log: ${LOG_FILE}${NC}\n"
        return 1
    }
