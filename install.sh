...
#######################################################
#  LINUX ON TERMUX - Multi-Distro Installer v3.3
#
#  Distributions:
...
step_install_distro() {
    ...
    if ! proot-distro list 2>/dev/null | grep -q "$PROOT_DISTRO"; then
        echo -e "  ${RED}✗${NC} Container '$PROOT_DISTRO' not found — install may have failed silently"
    ...
    if [ $rc -ne 0 ]; then
        ...
        proot_run_verbose "
            ...
            apt-get install -y --no-install-recommends \
                ...
                dolphin konsole kate ark spectacle okular \
